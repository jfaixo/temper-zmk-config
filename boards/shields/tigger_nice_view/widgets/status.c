...
static void draw_top(lv_obj_t *widget, lv_color_t cbuf[], const struct status_state *state) {
    ...
    // Modifiers
    char label_mods_value[5] = {' ', ' ', ' ', ' ', 0};
    if ((state->modifiers & MOD_LCTL) != 0) {
        label_mods_value[0] = 'C';
    }
    if ((state->modifiers & MOD_LSFT) != 0) {
        label_mods_value[1] = 'S';
    }
    if ((state->modifiers & MOD_LALT) != 0) {
        label_mods_value[2] = 'A';
    }
    if ((state->modifiers & MOD_LGUI) != 0) {
        label_mods_value[3] = 'G';
    }

    lv_draw_label_dsc_t label_mods_desc;
    init_label_dsc(&label_mods_desc, LVGL_FOREGROUND, &lv_font_unscii_16, LV_TEXT_ALIGN_CENTER);
    lv_canvas_draw_text(canvas, 0, 20, 68, &label_mods_desc, label_mods_value);
...
// #region Modifiers status
static void set_modifiers_status(struct zmk_widget_status *widget,
...
static struct modifiers_status_state modifiers_status_get_state(const zmk_event_t *eh) {
    ...
    return (struct modifiers_status_state){.modifiers = report->body.modifiers};
}
...
// Initialization of the split master screen
